...
<a href=""><img src="https://img.shields.io/badge/python-3.8+-aff.svg"></a> <a href=""><img src="https://img.shields.io/badge/Inc-Ssymmetry-blue"></a> <a href=""><img src="https://img.shields.io/badge/OS-Linux-green"></a> <a href=""><img src="https://img.shields.io/badge/CEO-Hengkui%20Wu-red" align="right"></a>
...
- 通过这一创新，T5模型在语言理解评测基准SuperGLUE上成功接近人类的表现。
- T5共有Base,Large,XL,XXL四个版本，分别对应220M,770M,3.3B和10B参数规模
...
- 为了使T5能够更好的理解中文金融领域的文本，我们爬取了海量公开数据，收集整理了百G规模的公司公告，研究报告，财经新闻与网络论坛数据，形成了中文金融语料库FinCorpus
- 在FinCorpus上预训练出的FinT5-Base，相对于普通文本上训练出来的T5，在金融评测任务上取得了显著提升。
- 在此基础上，我们还挖掘了知识增强和课程学习两大创新点：
    ...
    2. 课程学习：
        - 由易到难的学习策略主导了人类自幼儿时期起的学习过程，这是非常自然的。研究表明，使用由易到难的学习策略训练神经网络也能使其学习的更好。我们开创性的将课程学习思想引入大规模预训练语言模型，目前正在进行实验验证其有效性。


### FinCUGE
- 为了验证我们模型在金融NLP任务上的先进性，也为了填补行业标准的空白，我们搜集、整理和标注形成了包含八个数据集的中文金融自然语言理解与生成评测基准FinCUGE
- FinCUGE涵盖了关系抽取、事件抽取、事件主体识别、负面消息判定及负面主体判定、新闻标签分类、自动化新闻摘要，论坛情绪分析和命名实体识别八类任务
- 下面我们将会展开介绍FinCUGE
...
### 汇总
|model\dataset|FinRE关系抽取|FinESE事件主体|FinFE论坛情绪|FinNSJ负面主体|FinNA新闻摘要|FinNL新闻标签|FinRC阅读理解|FinNER实体识别|AVG|
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|T5-base|58.29|88.88|76.28|87.91|51.29|87.19| | |74.97|
|FinT5-base|62.32|**90.73**|**80.05**|88.50|52.00|91.18| | |77.46|
|FinT5-base-KS|63.59|90.39|79.23|**89.05**|**53.74**|**91.40**| | |**77.90**|
...
- 课程学习
    1. 随训练步骤的增长逐步提升训练的难度
    2. 难度通过两个维度控制：MASK 概率与MASK SPAN 长度
    3. 近期有研究表明，大模型对较高MASK比例更加适应
